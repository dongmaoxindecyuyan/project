...
public class AiMindMapController {
    ...
    public Flux<CommonResult<String>> generateMindMap(@RequestBody @Valid AiMindMapGenerateReqVO generateReqVO) {
        // 优化prompt为自然语言表达
        String prompt = generateReqVO.getPrompt().trim();
        String newPrompt = "请以" + prompt + "为主题，生成结构化的思维导图，使用Markdown格式，节点层级用 #、##、###、#### 表示。要求结构清晰、内容全面、不要添加任何解释文字。";
        generateReqVO.setPrompt(newPrompt);
